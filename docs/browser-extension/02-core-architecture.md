...
之前 `"persistent": true/false` 控制的后台页面，现在变成一个纯粹的事件驱动 Service Worker。详情在[Service Worker 层](04-service-worker.md)展开，这里只说架构层面的影响：
...
`webRequest` 还能监听，但不能再拦截和修改请求。代替方案是 Declarative Net Request——声明式规则引擎，具体在[网络层](07-network.md)展开。
